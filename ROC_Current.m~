...
fx = zeros(nGridPts, 1);
for a = 1:nGridPts - 1
    if x(a) >= pi && x(a) < -0.75*pi 
        fx(a) = 0;
    end
    if x(a) >= -0.75*pi && x(a) < -0.5*pi
        fx(a) = cos(2*x(a));
    end
    if x(a) >= -0.5*pi && x(a) < .5*pi
        fx(a) = cos(3*x(a)/2);
    end
    if x(a) >= .5*pi && x(a) < .75*pi
        fx(a) = cos(2*x(a));
    end
    if x(a)>= .75*pi && x(a) <=pi
    fx(a) = 0
...
wtf = exp(2.3*1i*fourModes'*(2-(3.7*10^(-9)*1i*fourModes')+exp(1.57*1i*fourModes')*(4.93*10^(-9)-1i*fourModes')));
...
pfa = 0.01;
%define a variable to hold the location of a jump
jump_detect = NaN(nGridPts, 1);
for a = 1:nGridPts
    %Define M
    M = zeros(3,1);
    Exponent_factor_1 = exp( 1i*(x(a)-delta) * fourModes.');
    Exponent_factor_2 = exp( 1i*(x(a)) * fourModes.');
    Exponent_factor_3 = exp( 1i*(x(a)+delta) * fourModes.');
    jmpFncCfs_1 = 1i * fHat .* sign(fourModes) .* cfac;
    jmpFncCfs_2 = 1i * fHat .* sign(fourModes) .* cfac;
    jmpFncCfs_3 = 1i * fHat .* sign(fourModes) .* cfac;
    ...
    cov = zeros(3);
    cov(1,1) = sum((rho2) * real(cfac.^2));
    cov(2,2) = sum((rho2) * real(cfac.^2));
    cov(3,3) = sum((rho2) * real(cfac.^2));
    cov(1,2) = sum((rho2) * real((cfac.^2).*exp(1i.*fourModes.* ((a-delta)-a))));
    cov(1,3) = sum((rho2) * real((cfac.^2).*exp(1i.*fourModes.* ((a-delta)-(a+delta)))));
    cov(2,1) = sum((rho2) * real((cfac.^2).*exp(1i.*fourModes.* (a-(a-delta)))));
    cov(2,3) = sum((rho2) * real((cfac.^2).*exp(1i.*fourModes.* (a-(a+delta)))));
    cov(3,1) = sum((rho2) * real((cfac.^2).*exp(1i.*fourModes.* ((a+delta)-(a-delta)))));
    cov(3,2) = sum((rho2) * real((cfac.^2).*exp(1i.*fourModes.* ((a+delta)-a))));
    %Define D 
    D = M*inv(cov)*M';
    %Calculate gamma_dot
    % gamma_dot = Qinv(pfa)*D;
    gamma_dot = .5;
    %define Y
    Y = zeros(3,1);
    Exponent_factor_1 = exp( 1i*(x(a)-delta) * fourModes.');
    Exponent_factor_2 = exp( 1i*(x(a)) * fourModes.');
    Exponent_factor_3 = exp( 1i*(x(a)+delta) * fourModes.');
    jmpFncCfs_1 = 1i * fHat_alt.* sign(fourModes) .* cfac;
    jmpFncCfs_2 = 1i * fHat_alt.* sign(fourModes) .* cfac;
    jmpFncCfs_3 = 1i * fHat_alt.* sign(fourModes) .* cfac;
    Y_1 = real(Exponent_factor_1 * jmpFncCfs_1);
    Y_2 = real(Exponent_factor_2 * jmpFncCfs_2);
    Y_3 = real(Exponent_factor_3 * jmpFncCfs_3);
    Y(1) = Y_1; Y(2) = Y_2; Y(3) = Y_3; 
    Y = Y';
    %Calculate test statistic
    test_stat = M*inv(cov)*Y';
    %compare test statistic to gamma to detect jump
    Exponent_factor_a = exp( 1i*x(a)*fourModes');
    jmpFncCfs_a = 1i * fHat.* sign(fourModes) .* cfac;
    if test_stat > gamma_dot
        jump_detect(a) = real(Exponent_factor_a * jmpFncCfs_a)
    else
        jump_detect(a) = NaN;
    end
end
plot(x, fx, 'r', x, jump_detect, 'go');
