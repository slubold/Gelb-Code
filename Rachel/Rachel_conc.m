...
%%---------------------------------------------------
function [sparse_jump_CF, higher_order_CF, problem_4_CF] = conc_design
clear all; 
close all; clc
...
problem_4_CF = zeros(2*N+1,1);
higher_order_CF = zeros(2*N+1,1);
sparse_jump_CF = zeros(2*N+1,1);
...
% This first cvx program will create the sparse jump response concentration
% factor.
cvx_begin
    ...
    subject to
         ...
         real(ker(x==0))==1;
         %norm(ker(abs(x)>=1),inf)<=1e-3;
cvx_end
sparse_jump_CF = cfac
%plot(x, sparse_jump_CF)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the bit where we call CVX to solve the variational formulation
% This first cvx program will create the higher-order kernel response concentration
% factor.
cvx_begin
    % conc. factor is an even function
    variable c_fac(N);
    ...
    subject to
         ...
         % for missing data
         %sig(ind)==0;
         %norm(imag(ker),inf)<=1e-4;
         
         %for higher order kernels/edge detectors
         norm(der_ker,inf)<=.1;
         norm(der_2ker,inf)<=.001;
         
         % other possible constraints
         sig>=0;
         sig(1)==0;
cvx_end
higher_order_CF = c_fac
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the bit where we call CVX to solve the variational formulation
% This first cvx program will create the higher-order kernel response concentration
